...
| **Background jobs** | Cron / delayed / queue-worker contracts touch persistence, scheduling, and observability — service code should not be coupled to Asynq, River, or Temporal directly. | `ports/jobs.Scheduler` + `ports/jobs.Worker` + `ports/jobs.Job`. Concrete drivers live in adapters. |
| **Inbound idempotency-key** | Distinct from `eventbus.Inbox` (which is broker-side dedup). HTTP POST retry protection needs an idempotency-key contract that can reuse `ports/cache` as backing. | `ports/idempotency.Store` + middleware contract (middleware itself lives in HTTP adapter). |
