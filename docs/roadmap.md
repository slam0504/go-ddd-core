...
| **Rate limiting** ✅ contract shipped (unreleased, tag-gate) | Inbound throttling and outbound quota share the same "may this call proceed?" shape. Token-bucket vs leaky-bucket vs sliding-window is an adapter concern. | `ports/ratelimit.Limiter.Allow(ctx, key) (Result, error)` — the decision is data (`Result.Allowed`), NOT an error; `Result` carries a `RetryAfter` conservative wait hint plus accurate-or-absent `Limit`/`Remaining`/`ResetAt` advisory metadata (`UnknownCount`/`IsZero` sentinels) — plus the deterministic-only `ratelimittest` suite. **Done in core**, awaiting first adapter consumer to cut the tag. Outbound quota / blocking wait deliberately deferred to a future httpclient-driven consumer; algorithm / config / storage are adapter concerns. |
